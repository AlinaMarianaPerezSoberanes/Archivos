...
int main(){
    ...
    guardarBMP("callesobe.bmp",&info,imagenRGB);
    free(imagenRGB); 
    free(imagenGray);
    free(imagenAux);    
}*/
...
void * funHilo(void *arg){
    ...
    int convC,pixel;
    unsigned int mascara[DIMASK*DIMASK]={1, 0,-1,
                                         2, 0,-2,
                                         1, 0,-1};
    unsigned int mascaraCol[DIMASK*DIMASK]={-1,-2,-1,
                                             0, 0, 0,
                                             1, 2, 1};
    int nh=*(int*)arg;
            ...
            for(x=0; x<info.width-DIMASK; x++){                             //recorrer mascara en la imagen
                ...
                for(ym=y; ym<y+DIMASK; ym++){                               //recorrer mascara para operaciones
                    for(xm=x; xm<x+DIMASK; xm++){                           //recorrer mascara con operaraciones
                        ...
                        convC+=imagenGray[indiceI] * mascaraCol[indiceM];       //convolucion
                    }                                                
                }
            conv/=4;
            convC/=4;
            pixel=sqrt(pow(convC,2)+pow(conv,2));                                                              //Dividir la mascar  
            indiceI=(y+1)*info.width+(x+1);
            imagenAux[indiceI]=pixel;
            }
        }
    pthread_exit(arg);
}
