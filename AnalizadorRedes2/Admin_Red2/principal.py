...
#Algoritmo de Linea de base
def lbase():
    ventanal = Toplevel(root)
    # Graficos
    ventanal.title("Linea Base")
    ventanal.configure(background="snow")
    # Ventana principal
    val = Frame(ventanal)
    val.grid(column=0, row=0, padx=(50, 50), pady=(5, 5))
    val.columnconfigure(0, weight=1)
    val.rowconfigure(0, weight=1)

    # Umbrales l base
    lbll = Label(val, text="Ingrese los umbrales", bg="dodger blue")
    lbll.grid(column=0, row=3, sticky=(W, E))
    lbl1 = Label(val, text="Umral MAX ")
    lbl1.grid(column=0, row=4)
    entradalb.append(Entry(val, width=20, textvariable=u1))
    entradalb[0].grid(column=1, row=4)

    lbl2 = Label(val, text="Umbral DESV ")
    lbl2.grid(column=0, row=5)
    entradalb.append(Entry(val, width=20, textvariable=u2))
    entradalb[1].grid(column=1, row=5)

    lbl3 = Label(val, text="Umbral MON ")
    lbl3.grid(column=0, row=6)
    entradalb.append(Entry(val, width=20, textvariable=u3))
    entradalb[2].grid(column=1, row=6)

    btn1 = Button(val, text="Iniciar", command=imp)
    btn1.grid(column=2, row=5)
    
def imp():
    print("Umbrales"+entradalb[0].get())
    print("Umbrales"+entradalb[1].get()) 
    print("Umbrales"+entradalb[2].get())     

#Algoritmo Minimos Cuadrados
#def min_cuad():
    #LLamar al hilo para realizar esta función

#Algoritmo Holt Winters
#def hw():
    #Llamar al hilo para iniciar la función
...
btn6=Button(master_agentes,text="Linea de Base",command=lbase,bg='azure3')
btn6.grid(column=1,row=1)
btn7=Button(master_agentes,text="Min Cuadrados",bg='azure3')
btn7.grid(column=1,row=2)
btn7=Button(master_agentes,text="Holt Winters",bg='azure3')
